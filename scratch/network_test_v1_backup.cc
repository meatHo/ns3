...
main(void)
{
    ...
    Ipv6InterfaceContainer iic1 = ipv6h.Assign(ndc1);
    iic1.SetForwarding(1, true);
    iic1.SetDefaultRouteInAllNodes(1);

    //router server
    ipv6h.SetBase("fd00:2::", Ipv6Prefix(64));
    Ipv6InterfaceContainer iic2 = ipv6h.Assign(ndc2);
    ...
    Ipv6InterfaceContainer iic3 = ipv6h.Assign(ndc3);
    ...
    // // 2. 목적지 UE의 IP 주소를 정의합니다.
    // Ipv6Address ueHostAddress("7777:f00d::");
    //
    // // 3. UE가 속한 네트워크의 Prefix를 정의합니다. (보통 /64 사용)
    // Ipv6Prefix ueNetworkPrefix(64);
    //
    // // 5. 서버의 정적 라우팅 테이블에 접근합니다.
    // Ptr<Ipv6StaticRouting> serverStaticRouting =
    //     ipv6RoutingHelper.GetStaticRouting(server->GetObject<Ipv6>());
    //
    // // 6. 라우팅 규칙을 추가합니다.
    // // "7777:f00d::/64 네트워크로 가려면, 1번 인터페이스로 패킷을 보내라"
    // serverStaticRouting->AddNetworkRouteTo(ueHostAddress, ueNetworkPrefix, 1);

    // // --- UE 네트워크 주소 알아내기 ---
    // Ipv6Address ueHostAddress("7777:f00d::");
    // Ipv6Prefix ueNetworkPrefix(64);
    // Ipv6Address ueNetworkAddress = ueHostAddress.CombinePrefix(ueNetworkPrefix);
    //
    // // --- 라우터의 라우팅 테이블에 경로 추가 ---
    // // pgw와 라우터가 연결된 링크(net3)에서 pgw의 IP 주소를 알아냅니다.
    // // pgw가 0번 인덱스이므로, pgw의 IP 주소는 iic3.GetAddress(0, 1) 입니다.
    // Ipv6Address pgwAddressOnNet3 = iic3.GetAddress(0, 1);
    //
    // Ptr<Ipv6StaticRouting> routerStaticRouting =
    //     ipv6RoutingHelper.GetStaticRouting(router->GetObject<Ipv6>());
    //
    // // router에게 "ueNetworkAddress로 가려면, pgw(pgwAddressOnNet3)에게 보내라"고 알려줍니다.
    // // 마지막 '2'는 router에서 pgw로 향하는 인터페이스 인덱스입니다. (토폴로지에 따라 확인 필요)
    // routerStaticRouting->AddNetworkRouteTo(ueNetworkAddress, ueNetworkPrefix, pgwAddressOnNet3, 2);


    // ipv6h.Assign(pgwToRouterNetDev);
    //
    // // RSU <--> Router 링크에 IP 할당
    // ipv6h.SetBase("fd00:2::", Ipv6Prefix(64));
    // ipv6h.Assign(rsuToRouterNetDev);
    //
    // // Router <--> Server 링크에 IP 할당
    // ipv6h.SetBase("fd00:3::", Ipv6Prefix(64));
    // Ipv6InterfaceContainer routerToServerIface = ipv6h.Assign(routerToServerNetDev);
    // Ipv6Address serverIpv6 = routerToServerIface.GetAddress(1, 0);

    // Ipv6InterfaceContainer pgwIface = ipv6h.Assign(pgwToServerNetDev);//두 연결된 디바이스들 ip
    // 할당 해줌 Ipv6InterfaceContainer rsuIface = ipv6h.Assign(rsuToServerNetDev);//근데 이
    // 방식이면 서버에 ip가 두개 할당 되어서 시나리오랑 다른듯
    ...
    clientAppsUe.Start(Seconds(6.0));
    ...
    clientAppsRsu.Start(Seconds(11.0));
